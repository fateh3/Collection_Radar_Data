try:
    from numpy import radarWrapper
...
class CollectionThread(threading.Thread):
    def __init__(self, threadID, name, radarBuffer, stopEvent,radarSettings, radarIP='192.168.7.2', simulate = False, filePaths = None,
...
class CollectionThreadX4(threading.Thread):
    def __init__(self, threadID, name, radarBuffer, stopEvent, radarSettings, baseband=False, fs=17,
                 radarPort='/dev/tty.usbmodem14301', simulate=False, filePaths=None,
                 nonRealTimeMode=False,resumeEvent=None):
        threading.Thread.__init__(self)
        self.name = name
        self.threadID = threadID
